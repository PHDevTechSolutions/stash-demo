/**
 * pages/api/vault/copy.ts
 * POST — decrypt and return the plaintext password for clipboard copy.
 *        The UI writes it to the clipboard; it is never rendered in the DOM.
 *        Logs a 'copy' audit event.
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { data: entry, error } = await supabase
      ...
      .select("id, password_encrypted, iv, referenceid")
      .eq("id", entryId)
      .eq("is_active", true)
      .single();
    ...
    try {
      ...
      console.error("[vault/copy] Decryption failed:", decryptErr.message);
    ...
    console.error("[vault/copy]", err.message);
